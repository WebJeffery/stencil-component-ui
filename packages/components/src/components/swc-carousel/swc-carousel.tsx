...
@Component({
  tag: 'swc-carousel',
  styleUrl: 'swc-carousel.css',
  shadow: true,
})
export class SwcCarousel {
  ...
  componentWillLoad() {
    writeTask(() => {
      this.slotChildren = findElementsDownward(this.root, 'swc-carousel-item');
      // this.slotChildren.forEach(c => {
      //   console.log(c.assignedSlot);
      // });
      // const tmp = (this.slotEl as any).assignedElements();
      // console.log(tmp);
      const carouselItemList = this.root.querySelectorAll('swc-carousel-item');
